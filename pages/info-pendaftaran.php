...
<style>
  ...
  .dokumen-card { background:var(--bg-white); border:1px solid var(--border); border-radius:var(--radius-md); padding:1.1rem 1.25rem; display:flex; align-items:flex-start; gap:1rem; transition:border-color 0.18s, box-shadow 0.18s; }
  .dokumen-card:hover { border-color:var(--primary-mid); box-shadow:var(--shadow-md); }
  .dokumen-icon { width:42px; height:42px; border-radius:var(--radius-sm); display:flex; align-items:center; justify-content:center; font-size:1rem; flex-shrink:0; }
  ...
  .jadwal-wrap::before { content:''; position:absolute; left:10px; top:0; bottom:0; width:2px; background:linear-gradient(to bottom, var(--primary), var(--border)); }
  .jd-item { position:relative; padding-bottom:1.75rem; }
  .jd-item:last-child { padding-bottom:0; }
  .jd-dot { position:absolute; left:-2rem; top:2px; width:22px; height:22px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:0.65rem; border:3px solid var(--bg-white); }
  ...
  .jd-body { background:var(--bg-white); border:1px solid var(--border); border-radius:var(--radius-md); padding:0.9rem 1.1rem; margin-left:0.5rem; }
  ...
  .countdown-box {
    ...
    border-radius:var(--radius-lg); padding:1.25rem 1.5rem; color:#fff;
...
<div class="container-dashboard-orangtua">
  ...
  <div class="content-isi">
    ...
    <div class="tab-nav">
      <button class="tab-btn active" onclick="switchTab('syarat',this)"><i class="fas fa-file-check"></i> Syarat & Dokumen</button>
      <button class="tab-btn" onclick="switchTab('jadwal',this)"><i class="fas fa-calendar-alt"></i> Jadwal Pendaftaran</button>
    </div>
    ...
    <div class="tab-panel active" id="tab-syarat">
      ...
      <div class="info-box">
        ...
        <div><strong>Catatan penting:</strong> Semua dokumen wajib diserahkan dalam bentuk fotokopi <strong>dan</strong> dokumen asli untuk diverifikasi. Pastikan semua dokumen dalam kondisi jelas dan tidak rusak.</div>
      </div>
    </div>
    ...
    <div class="tab-panel" id="tab-jadwal">
      ...
      <div class="countdown-box">
        <div>
          ...
          <div style="font-size:1rem;font-weight:700;">30 Juni 2026</div>
        </div>
        <div class="cd-nums">
      ...
      <div class="jadwal-wrap">
        ...
        <div class="jd-item" id="tahap-sosialisasi">
          <div class="jd-dot pending" id="dot-sosialisasi"><i class="fas fa-bullhorn"></i></div>
          <div class="jd-body">
            <span class="badge badge-gray" id="badge-sosialisasi" style="margin-bottom:6px;">Akan Datang</span>
            <h4>Sosialisasi & Pengumuman PPDB</h4>
            <div class="jd-date"><i class="fas fa-calendar"></i> 1 Januari – 28 Februari 2026</div>
            <p>Penyebaran informasi PPDB melalui website, media sosial, dan pengumuman di sekolah.</p>
          </div>
        </div>

        <div class="jd-item" id="tahap-pengumuman">
          <div class="jd-dot pending" id="dot-pengumuman"><i class="fas fa-bullhorn"></i></div>
          <div class="jd-body">
            <span class="badge badge-gray" id="badge-pengumuman" style="margin-bottom:6px;">Akan Datang</span>
            <h4>Pendaftaran PPDB</h4>
            <div class="jd-date"><i class="fas fa-calendar"></i> 1 Maret – 30 Juni 2026</div>
            <p>Panitia memverifikasi kelengkapan dan keabsahan berkas pendaftaran.</p>
          </div>
        </div>

        <div class="jd-item" id="tahap-pendaftaran">
          <div class="jd-dot pending" id="dot-pendaftaran"><i class="fas fa-folder-open"></i></div>
          <div class="jd-body">
            <span class="badge badge-gray" id="badge-pendaftaran" style="margin-bottom:6px;">Akan Datang</span>
            <h4>Pengumpulan Berkas & Verifikasi</h4>
            <div class="jd-date"><i class="fas fa-calendar"></i> 1 Juli – 10 Juli 2026</div>
            <p>Calon peserta didik mendaftarkan diri secara online dan mengumpulkan dokumen.</p>
          </div>
        </div>

        <div class="jd-item" id="tahap-persiapan">
          <div class="jd-dot pending" id="dot-persiapan"><i class="fas fa-school"></i></div>
          <div class="jd-body">
            <span class="badge badge-gray" id="badge-persiapan" style="margin-bottom:6px;">Akan Datang</span>
            <h4>Persiapan Tahun Ajaran Baru</h4>
            <div class="jd-date"><i class="fas fa-calendar"></i> 25 Mei 2026</div>
            <p>Kegiatan belajar mengajar tahun ajaran 2026/2027 resmi dimulai.</p>
          </div>
        </div>
...
function cekTahapan() {
  ...
  const tahapan = [
    {
      id: 'sosialisasi',
      mulai:   new Date('2026-01-01T00:00:00'),
      selesai: new Date('2026-02-28T23:59:59'),
    },
    {
      id: 'pengumuman',
      mulai:   new Date('2026-03-01T00:00:00'),
      selesai: new Date('2026-06-30T23:59:59'),
    },
    {
  id: 'pendaftaran',
  mulai:   new Date('2026-07-01T00:00:00'),
  selesai: new Date('2026-07-10T23:59:59'),
  },
    {
      id: 'persiapan',
      mulai:   new Date('2026-05-25T00:00:00'),
      selesai: new Date('2026-05-25T23:59:59'),
    },
  ];

  tahapan.forEach(t => {
    const wrap  = document.getElementById('tahap-' + t.id);
    const dot   = document.getElementById('dot-' + t.id);
    const badge = document.getElementById('badge-' + t.id);

    if (now >= t.mulai && now <= t.selesai) {
      // Sedang berlangsung — aktifkan animasi spin
      wrap.classList.add('current');
      dot.className = 'jd-dot current';
      dot.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
      badge.className = 'badge badge-yellow';
      badge.textContent = 'Berlangsung';
    } else if (now > t.selesai) {
      // Sudah selesai
      dot.className = 'jd-dot done';
      dot.innerHTML = '<i class="fas fa-check"></i>';
      badge.className = 'badge badge-green';
      badge.textContent = 'Selesai';
    }
    // Kalau belum mulai, biarkan default (pending / Akan Datang)
  });
}
cekTahapan();
</script>
